...
class Snail {
  ...
  render () {
    const snailRacerElm = document.createElement('div');
    snailRacerElm.className = `snail-racer`;
    snailRacerElm.innerHTML = `
  ...
  <div class="snail-racer__name">${this.name}</div>
    ...
    const colorSnailElm = snailRacerElm.querySelector('.snail__shell');
    colorSnailElm.style.fill = `${this.color}`;
  ...
  update() {
    let curentSpeed = Number(this.speed);
    setInterval(() => {
      curentSpeed += Number(this.speed)/25;
      this.element.style.marginLeft = `${curentSpeed}px`;
...
fetch(`http://bootcamp.podlomar.org/api/snails/5`)
  ...
  .then((json) => {
    ...
    for(let i = 0; i < json.length; i += 1){
      ...
      arrSnails[i] = new Snail(`${json[i].name}`, `${json[i].velocity}`, `${json[i].color}`);
      arrSnails[i].mount(app);
    }
    console.log(arrSnails);
